...
Answer :Context API helps sharing data within different components without the need to pass data through props.

2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

Answer :actions are events that describe something that will happen in the application , reducers are function that take the current state and actions and return new state and tells (the store) how that state is changed. The store is the object that holds the data and is the only place the data or the state is placed.

3. What does `redux-thunk` allow us to do? How does it change our `action-creators`?

Answer : Redux-Thunk middleware allows you to create action creators that return a function(thunk) instead of an action object.The thunk action takes the store's dispatch method as an argument which afterwards it can be used to delay a dispatch of an action or to dispatch only if a certain condition is met.

4. What is your favorite state management system you've learned and this sprint? Please explain why!

Answer-My favorite state management is Context API because its easy to set up easy to modify and you can pass data between Components without having to explicitly pass props from every Parent component to the Child Component.
